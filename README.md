...
`iddx_core` is designed for parsing `.iddx` and `.out` files from **trusted sources** (your own InfoDrainage projects, files from collaborators, files generated by simulation runs you control).
...
| Class                 | Description                                                                                                                                                                    |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `IddxModel`           | Top-level model. `IddxModel.open()` to read, `.save()` to write, `.new()` to create from scratch. Supports `with` for context-manager use.                                     |
| `Phase`               | A design scenario containing all network elements. Access via `model.phases["name"]`. `find_*(label)` returns `None` if missing; `get_*(label)` raises `ElementNotFoundError`. |
| `Catchment`           | Inflow area. Key properties: `.label`, `.area`, `.cv` (alias `.runoff_coefficient`), `.pimp` (alias `.percent_impervious`), `.runoff_method`                                   |
