...
class ResultsReader:
    ...
    def _read_string(self, offset: int) -> tuple[str, int]:
        ...
        s = self._data[offset + 4 : offset + 4 + slen].decode("ascii", errors="replace")
    ...
    def _parse_variables(self):
        ...
        off += 4
        self._node_var_codes = struct.unpack_from(f"<{self._n_node_vars}i", self._data, off)
        off += self._n_node_vars * 4

        self._n_link_vars = struct.unpack_from("<i", self._data, off)[0]
        off += 4
        self._link_var_codes = struct.unpack_from(f"<{self._n_link_vars}i", self._data, off)
    ...
    def _period_time(self, period: int) -> datetime.datetime:
        ...
        return datetime.datetime(1899, 12, 30) + datetime.timedelta(days=days, seconds=seconds)
    ...
    def system_variables(self) -> list[str]:
        return [SYSTEM_VARIABLES.get(i, f"sys_var_{i}") for i in range(self._n_sys_vars)]
    ...
    def node_summary(self, node_id: str, label: str = "") -> NodeSummary:
        ...
        for p in range(self._n_periods):
            if "depth_above_invert" in var_names:
                v = self._node_result(idx, var_names.index("depth_above_invert"), p)
                if v > summary.peak_depth:
                    summary.peak_depth = v

            if "hydraulic_head" in var_names:
                v = self._node_result(idx, var_names.index("hydraulic_head"), p)
                if v > summary.peak_head:
                    summary.peak_head = v

            if "lateral_inflow" in var_names:
                v = self._node_result(idx, var_names.index("lateral_inflow"), p)
                if v > summary.peak_lateral_inflow:
                    summary.peak_lateral_inflow = v

            if "total_inflow" in var_names:
                v = self._node_result(idx, var_names.index("total_inflow"), p)
                if v > summary.peak_total_inflow:
                    summary.peak_total_inflow = v

            if "flooding" in var_names:
                v = self._node_result(idx, var_names.index("flooding"), p)
    ...
    def link_summary(self, link_id: str, label: str = "") -> LinkSummary:
        ...
        for p in range(self._n_periods):
            if "flow_rate" in var_names:
                v = self._link_result(idx, var_names.index("flow_rate"), p)
                if abs(v) > abs(summary.peak_flow):
                    summary.peak_flow = v
                    summary.time_of_peak_flow = self._period_time(p)

            if "flow_depth" in var_names:
                v = self._link_result(idx, var_names.index("flow_depth"), p)
                if v > summary.peak_depth:
                    summary.peak_depth = v

            if "flow_velocity" in var_names:
                v = self._link_result(idx, var_names.index("flow_velocity"), p)
                if v > summary.peak_velocity:
                    summary.peak_velocity = v

            if "capacity" in var_names:
                v = self._link_result(idx, var_names.index("capacity"), p)
                if v > summary.max_capacity:
                    summary.max_capacity = v

        return summary

    def all_node_summaries(self, label_map: Optional[dict[str, str]] = None) -> list[NodeSummary]:
    ...
    def all_link_summaries(self, label_map: Optional[dict[str, str]] = None) -> list[LinkSummary]:
...
class ScenarioComparison:
    ...
    def __init__(self, results: list[ScenarioResult], label_map: Optional[dict[str, str]] = None):
    ...
    def link_comparison(self, variable: str = "peak_flow") -> list[dict]:
        ...
        for r in self.results:
            for lid in r.link_peaks:
                if lid not in all_link_ids:
                    all_link_ids.append(lid)
    ...
    def nodes_to_csv(self, filepath: str | Path, variable: str = "peak_flooding") -> Path:
        """Export node-level comparison to CSV."""
        import csv
        filepath = Path(filepath)
        table = self.node_comparison(variable)
        if not table:
            return filepath
    ...
    def links_to_csv(self, filepath: str | Path, variable: str = "peak_flow") -> Path:
        ...
        import csv
        filepath = Path(filepath)
        table = self.link_comparison(variable)
        if not table:
            return filepath
